...
import ru.erked.pcook.ai.ISMCTS;
...
public class FightAI extends AdvScreen {
    ...
    private GameAction computed;
    private ISMCTS AI;
    public static float percents = .0f;
    ...
    private ArrayList<AdvSprite> loading_regions;
    ...
    public void render (float delta) {
        ...
        /* AI computations */
        if (!is_computing && turn % 2 == 0 && turn != 0) {
            new Thread(
                    () -> {
                        is_computing = true;
        ...
        /* Hiding the gems */
        if (turn % 2 == 0) {
            for (int i = 0; i < loading_regions.size() * percents; ++i) {
                if (!loading_regions.get(i).hasActions())
                    loading_regions.get(i).addAction(
                            Actions.sequence(
            ...
            for (AdvSprite s : s_hand_player)
                if (s != null && !s.hasActions())
                    s.addAction(Actions.color(Color.DARK_GRAY, 1f));
        } else {
            for (int i = 0; i < loading_regions.size(); ++i) {
                loading_regions.get(i).addAction(
                        Actions.sequence(
                                Actions.alpha(0f, .5f),
            ...
            for (AdvSprite s : s_hand_player)
                if (s != null && !s.hasActions())
                    s.addAction(Actions.color(Color.WHITE, 1f));
        }

        /* End of the game */
        if (!change_screen && turn != 0 && notifs.size() == 0 && !isNotTerminal()) {
                        ...
                        if (turn % 2 != 0) {
                            if (finalI - board_size > -1 &&
                                ...
                                applyAction(new GameAction(
                                        ...
                                        finalI,
                                        selected_hand_player));
                            } else if (finalI + board_size < board_size * board_size &&
                                    board_gems.get(finalI + board_size) != null &&
                                    isApplicable(new GameAction(
                                ...
                                applyAction(new GameAction(
                                        ...
                                        finalI,
                                        selected_hand_player));
                            } else if (finalI + 1 < board_size * board_size &&
                                    board_gems.get(finalI + 1) != null &&
                                    isApplicable(new GameAction(
            ...
            move_regions.get(i).addListener(new ClickListener() {
                ...
                public void clicked(InputEvent event, float x, float y) {
                    if (!move_regions.get(finalI).hasActions()) {
                        if (finalI - (selected_board_player) == 2 * board_size) {
                            applyAction(new GameAction(
                                    ...
                                    -1,
                                    -1));
                        } else if (finalI - (selected_board_player) == -2 * board_size) {
                            applyAction(new GameAction(
                                    ...
                                    -1,
                                    -1));
                        } else if (finalI - (selected_board_player) == 2) {
                            applyAction(new GameAction(
                        ...
                        if (selected_hand_player == -1) {
                            ...
                            for (int i = 0; i < board_gems.size(); ++i) {
                                if (board_gems.get(i) != null && !board_gems.get(i).isIsAI()) {
                                    if (isApplicable(new GameAction(
                                            ...
                                            i + board_size,
                                            selected_hand_player))) {
                                        place_regions.get(i + board_size).addAction(
                                                Actions.sequence(
                                                        Actions.visible(true),
                                    ...
                                    if (isApplicable(new GameAction(
                                            ...
                                            i - 1,
                                            selected_hand_player))) {
                                        place_regions.get(i - 1).addAction(
                                                Actions.sequence(
                                                        Actions.visible(true),
                                                        Actions.alpha(1f, .125f)
                                                )
                            ...
                            for (int i = 0; i < board_gems.size(); ++i) {
                                if (board_gems.get(i) != null && !board_gems.get(i).isIsAI()) {
                                    ...
                                    } else if (place_regions.get(i + board_size).isVisible()) {
                                        place_regions.get(i + board_size).addAction(
                                                Actions.sequence(
                                                        Actions.alpha(0f, .125f),
                                                        Actions.visible(false)
                                    ...
                                    } else if (place_regions.get(i - 1).isVisible()) {
                                        place_regions.get(i - 1).addAction(
                                                Actions.sequence(
                                                        Actions.alpha(0f, .125f),
                                                        Actions.visible(false)
    ...
    private void texture_initialization () {
        ...
        loading_regions = new ArrayList<>();
        for (int i = 0; i < 10; ++i) {
            loading_regions.add(new AdvSprite(
                    g.atlas.createSprite("loading"),
                    board.getX() + (board.getWidth() / 10f) * i,
                    board.getTop() + board.getWidth() / 40f,
                    board.getWidth() / 10f,
                    board.getWidth() / 20f
            ));
            loading_regions.get(i).addAction(Actions.alpha(0f));
            loading_regions.get(i).setVisible(false);
        }
    ...
    private boolean isApplicable (GameAction gameAction) {
        switch (gameAction.getType()) {
            case DISCARD: {
                return turn % 2 == 0 ? gameAction.getHandPosition() != -1 : selected_hand_player != -1;
            }
            case ADD_TO_AI_HAND: {
    ...
    private void applyAction (GameAction gameAction) {
        switch (gameAction.getType()) {
            case DISCARD: {
                if (gameAction.getHandPosition() != -1) {
                    cont_ai.get(gameAction.getHandPosition()).setColor(Color.WHITE);

                    s_hand_ai.get(gameAction.getHandPosition()).addAction(
                            Actions.moveTo(0 - 1.5f * gem_size, .5f * g.h - .5f * gem_size, .5f)
                    );

                    s_hand_ai.set(gameAction.getHandPosition(), null);
                    hand_ai.set(gameAction.getHandPosition(), null);
            ...
            case PLACE_UP: case PLACE_DOWN: case PLACE_LEFT: case PLACE_RIGHT: {
                ...
                if (turn % 2 == 0) {
                    board_gems.set(gameAction.getBoardPosition2(), hand_ai.get(gameAction.getHandPosition()));
                    hand_ai.set(gameAction.getHandPosition(), null);

                    s_board_gems.set(gameAction.getBoardPosition2(), s_hand_ai.get(gameAction.getHandPosition()));
                    s_hand_ai.set(gameAction.getHandPosition(), null);

                    s_board_gems.get(gameAction.getBoardPosition2()).addAction(
                            Actions.moveTo(
                                    board.getX() + (1f / 34f) * board_tile +
                                            (gameAction.getBoardPosition2() % board_size) * board_tile,
                                    board.getY() + (1f / 34f) * board_tile +
                                            (gameAction.getBoardPosition2() / board_size) * board_tile,
                                    .5f
                            )
                    ...
                    board_gems.set(gameAction.getBoardPosition2(), hand_player.get(selected_hand_player));
                    hand_player.set(selected_hand_player, null);

                    s_board_gems.set(gameAction.getBoardPosition2(), s_hand_player.get(selected_hand_player));
                    s_hand_player.set(selected_hand_player, null);
                    ...
                    s_board_gems.get(gameAction.getBoardPosition2()).clearListeners();
                    s_board_gems.get(gameAction.getBoardPosition2())
                            .addListener(board_listeners_player.get(gameAction.getBoardPosition2()));
            ...
            case MOVE_UP: {
                ...
                board_gems.set(
                        ...
                        board_gems.get(turn % 2 == 0 ? gameAction.getBoardPosition2() : selected_board_player));

                board_gems.set(turn % 2 == 0 ? gameAction.getBoardPosition2() : selected_board_player, null);

                s_board_gems.set(gameAction.getBoardPosition1(), s_board_gems.get(turn % 2 == 0 ? gameAction.getBoardPosition2() : selected_board_player));
                s_board_gems.set(turn % 2 == 0 ? gameAction.getBoardPosition2() : selected_board_player, null);
                ...
                if (turn % 2 != 0) {
                    ...
                    s_board_gems.get(gameAction.getBoardPosition1()).addListener(
                            board_listeners_player.get(gameAction.getBoardPosition1())
                    );
                }
    ...
    private boolean isNotTerminal() {
        ...
        if (turn % 2 == 0) {
            for (int i = 0; i < board_gems.size(); ++i) {
                if (board_gems.get(i) != null && board_gems.get(i).isIsAI()) {
                    for (int j = 0; j < hand_ai.size(); ++j) {
                        if (hand_ai.get(j) != null) {
            ...
            for (int i = 0; i < board_gems.size(); ++i) {
                if (board_gems.get(i) != null && !board_gems.get(i).isIsAI()) {
                    for (int j = 0; j < hand_player.size(); ++j) {
                        if (hand_player.get(j) != null) {
                            GameAction place_up = new GameAction(
                                    GameAction.Type.PLACE_UP,
                                    hand_player.get(j),
                                    i,
    ...
    private void stage_addition () {
        ...
        for (AdvSprite s : loading_regions) stage.addActor(s);
